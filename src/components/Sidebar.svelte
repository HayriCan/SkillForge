<script lang="ts">
  ...
  const mainItems = $derived.by(() => {
    ...
    const all = [
      ...
      { id: 'insights', label: t('nav.insights'),  icon: 'insights',  shortcut: undefined },
  ...
  >
    ...
    <span class="w-4 text-center shrink-0">
      ...
      {:else if item.icon === 'insights'}
        <svg class="w-3.5 h-3.5 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 13.5V18m4.5-9v9M12 9v9m4.5-13.5V18M21 3v15"/></svg>
